# Piggy Inventory

A simple yet powerful mod designed to enhance your inventory management experience in Minecraft. Piggy Inventory aims to streamline your interactions with items, tools, and containers.

## ⚠️ Disclaimer

**This is a personal project.** It comes **AS IS** and might have issues.
*   **No Support:** Do not expect regular support or bug fixes.
*   **No Forge Port:** There are **NO** plans to port this mod to Forge/NeoForge.
*   **Use at your own risk.**

Feel free to fork the project or submit a Pull Request if you want to contribute fixes or features!

---

## Features

### 🛠️ Current Features
*   **Automatic Tool Swapping**: Automatically swaps to the most effective tool in your hotbar depending on the block you are looking at. Never break stone with your fist again!

### � Future Features (Planned)
*   **Weapon Swapping**: Automatically switch to your weapon when attacking.
*   **Inventory Sorting**: Sort your inventory and containers with a single click.
*   **Fast Loot**: Quickly move items from a container to your inventory.
*   **Fast Deposit**: Quickly dump matching items into a container.
*   **Mouse Actions**: Enhanced mouse interactions within inventory GUIs for faster management.
*   **Tool Belt**: An extra inventory slot for tools (requires the mod to be installed on the server).
*   **Nearby Inventory Tabs**: Access surrounding containers via tabs in your inventory screen.

---

## Dependencies & Installation

### Requirements
*   **Minecraft**: ~1.21.1
*   **Fabric Loader**: >=0.18.1
*   **Java**: >=21

### Required Mods
*   **[Fabric API](https://modrinth.com/mod/fabric-api)**
*   **[YACL (Yet Another Config Lib)](https://modrinth.com/mod/yacl)**
*   *(Optional)* **[Mod Menu](https://modrinth.com/mod/modmenu)** - Highly recommended for accessing the configuration screen.

### Installation
1.  Download the `.jar` file.
2.  Install Fabric Loader for Minecraft 1.21.1.
3.  Place the `piggy-inventory` jar (along with Fabric API and YACL) into your `.minecraft/mods` folder.
4.  Launch the game!

---

**License**: CC0-1.0
